 ...
 * 11: black => magnet position motor, negative to increase eddy current effect, L293D OUT2
 * 12: red => magnet position motor, positive to increase eddy current effect, L293D OUT1
 * 
 * The resistance is generated by exploiting Lenz's law, which describes force induced
 * on a conductor by eddy current, due to a changing magnetic field. Turing the pedal 
 * crank spins a heavy aluminum flywheel, which has a series of strong permanent magnets 
 * mounted near the rim, with a small motor to move the magnets closer or further from
 * the rim of the flywheel, increasing or decreasing the eddy current drag force on
 * the rotation of the flywheel. A potentiometer is included to provide a feedback signal 
 * roughly proportional to the distance between the magnets and the wheel rim (position
 * sensor) which indicates the amount of drag on the wheel rim (resistance).
 * 
 ...
 * calibration for this machine, but could incorporate a calibration adjustment for the 
 * 100% position to be set by a technician, using the calibration potentiometer in the 
 ...
 * 
 * A trip timer is used to accumulate totals for distance, energy, average speed, 
 ...
 * provide for potential servo behavior. A slower timed update is used to accumulate trip totals
...
#define UP_MOTOR_SPEED 30 // pwm on duration out of 256, e.g. 30/256
...
time_t current_ms = millis();
time_t last_update_ms = current_ms; // start with Tlast=Tcurrent so there is an initial update interval measurement

// variables for trip duration
time_t trip_ms = 0; // trip interval time base, starts at 0 for each trip and increments by UPDATE_INTERVAL_MS on each update
unsigned trip_seconds = 0; // scaled from trip_ms
unsigned display_hours = 0; // hours to display hh:mm:ss
unsigned display_mins = 0; // minutes to display hh:mm:ss
unsigned display_sec = 0; // seconds to display hh:mm:ss
...
bool pedal_switch_closed = false;
time_t pedal_ms = millis();
time_t last_pedal_ms = pedal_ms-100000; // start with a 100 second interval

bool trip_reset_pressed = false;
...
void do_update() {
  ...
  for (update_index=0; update_index < sizeof(updates)/sizeof(update_record); update_index++) {
    update_record rec = updates[update_index]; // could use references to avoid copying
...
void loop() {
  ...
  // rollover-safe update activation interval
